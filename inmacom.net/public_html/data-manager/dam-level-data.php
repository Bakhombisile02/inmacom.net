...
<body>
  ...
  <div class="content ht-100v pd-0">
          ...
          <div>
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                ...
                <li class="breadcrumb-item active" aria-current="page">Manage Dam Levels</li>
              </ol>
            </nav>
            <h4 class="mg-b-0 tx-spacing--1">Manage Dam Level Data</h4>
          </div>
    ...
    <i data-feather="upload" class="wd-10 mg-r-5"></i> Export
        ...
        <div class="row row-xs">
          <div class="col-lg-3">
            <h6 class="mg-b-5">Add Dam Levels</h6>
            <form method="POST" id="dam_level_form">
              <div class="form-group">
                <label for="station" class="d-block">Dam</label>
                <select name="station" id="station-list" class="form-control select2">
                  <option selected>Select Dam</option>
                </select>
              </div>
              <div class="form-row">
                <div class="form-group col-md-6">
                  <label for="fsc" class="d-block">FSC (MCM)</label>
                  <input name="fsc" id="fsc" type="number" step=".01" class="form-control" placeholder="Enter value">
                </div>
                <div class="form-group col-md-6">
                  <label for="storage" class="d-block">% Storage</label>
                  <input name="storage" id="storage" type="number" step=".01" class="form-control" placeholder="Enter value">
                </div>
          ...
          <div class="col-lg-9">
            <table class="table" id="dam_level">
              <thead>
                <tr>
                  <th>Code</th>
  ...
  <div class="modal fade" id="import" tabindex="-1" role="dialog" aria-labelledby="importModal" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content tx-14">
        <div class="modal-header">
          <h6 class="modal-title" id="importModal">Import Dam Levels Data</h6>
    ...
    <a href="./templates/dam_levels.csv" target="_blank" class="btn btn-xs pd-x-15 btn-success btn-uppercase mg-l-5">
...
function updateLabel(input) {
  ...
  <div class="modal fade" id="edit_form" tabindex="-1" role="dialog" aria-labelledby="editModal" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content tx-14">
        <div class="modal-header">
        ...
        <div class="modal-body">
          <form method="POST" id="edit_dam_level_form">
            <div class="form-group">
              <label for="station" class="d-block">Dam</label>
              <select name="station" id="edit_station-list" class="form-control select2"></select>
            </div>
            <div class="form-row">
              <div class="form-group col-md-6">
                <label for="fsc" class="d-block">FSC (MCM)</label>
                <input name="fsc" id="edit_fsc" type="number" step=".01" class="form-control" placeholder="Enter value">
              </div>
              <div class="form-group col-md-6">
                <label for="storage" class="d-block">% Storage</label>
                <input name="storage" id="edit_storage" type="number" step=".01" class="form-control" placeholder="Enter value">
              </div>
            </div>
          </form>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-danger tx-13" data-dismiss="modal">Cancel</button>
          <button type="button" class="btn btn-primary tx-13 save-edit">Save</button>
        </div>

      </div>
    </div>
  </div>
  <div class="modal fade" id="delete-dam-level" tabindex="-1">
  ...
  <script>
    ...
    // get list of dams and populate dropdown
    getStations();
    $('.select2').select2({
    ...
    function Export() {
      $("#dam_level").table2excel({
        type: "csv",
        ignoreColumns: "#action",
        filename: "dam_level.csv"
      });
    }
    $('#dam_level_form').submit(function(e) {
      e.preventDefault();

      $.ajax({
        type: "POST",
        url: '../api/dam-level-data.php',
        data: $(this).serialize(),
        dataType: "json",
    ...
    // click event listner for edit button
    $(document).on('click', '.save-edit', function(e) {
      var id = $(this).data('id');
      var station = $('#edit_station-list').val()
      var fsc = $('#edit_fsc').val()
        ...
        success: function(response) {
          ...
          if (resp == "successful") {
            ...
            for (i = 0; i < data.length; i++) {
              ...
              //$("#edit_station-list").val(data[i].id);
              $("#edit_fsc").val(data[i].fsc);
              $("#edit_storage").val(data[i].value);
            }
...
$(document).ready(function() {
    $('#export_button').on("click", function(e) {
        ...
        $.ajax({
            url: "../api/export-dam-levels.php",
            method: "GET",
            success: function(data, status, xhr) {
                // Check for a filename
                var disposition = xhr.getResponseHeader('Content-Disposition');
                var matches = /"([^"]*)"/.exec(disposition);
                var filename = (matches != null && matches[1] ? matches[1] : 'dam_levels.csv');
